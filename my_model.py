...
class my_model1(nn.Module):
    ...
    def forward(
        ...
        return logits
class my_model_alignment(nn.Module):
    def __init__(self, config):
        super().__init__()
        semantic_embed = np.load("E:\data\iemocap\glove300d_full.npy")
        #print("semantic embeded shape",semantic_embed.shape)
    ...
    def forward(
        ...
        alpha = semantic_embed.matmul(acoustic_embed)

        soft_max_a = torch.nn.functional.softmax(alpha/math.sqrt(256), dim=1)
        #print(soft_max_a.transpose(2,1).shape)
        acoustic_embed=acoustic_embed.matmul(soft_max_a.transpose(2,1))

        acoustic_embed = acoustic_embed.transpose(2, 1)
        # then we use the cross modal excitement information
